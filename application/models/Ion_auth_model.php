...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		// prepare default section 

		$query = $this->db->query('SELECT id,name,display_order,status  FROM  tbl_main_section WHERE 1 ORDER BY display_order ASC');
		...
		foreach ($query->result() as $row) {
			$data = array(
				...
				'status' => $row->status,
				'recordAddedOn' => date("Y-m-d H:i:s"),
				'recordAddedByAdminId' => $id
			);

			$this->db->insert('tbl_section_choice', $data);
		}
